...
def load_target_names():
  target_names = ['H.nana', 'H.diminuta', 'Ancilostomideo', 'E.vermicularis', 'A.lumbricoides', 'T.trichiura', 'S.mansoni','Taenia parasites', 'impurities']
  
  return target_names


 
def  load_dataset_imagenet_preprocessing(num_channels, training_set_path, testing_set_path, nclass, img_rows):
  basewidth=img_rows
  #path for the images
  src="/home/gg/Eggs/pre/orig/"
  #==================Gera training Set
...
def  load_dataset_theano_imagenet_preprocessing(num_channels, training_set_path, testing_set_path, nclass, img_rows):
  basewidth=img_rows
...
def  load_dataset_without_pretrain(num_channels, training_set_path, testing_set_path, nclass, img_rows):
  basewidth=img_rows
  src="/home/osaku/Eggs/pre/orig/"
  #==================Gera training Set
  n_train = n_samples(training_set_path)
...
#====================================================
  ...
  X1_train = np.arange(n_train*basewidth*basewidth*3)
  X1_train.resize(n_train,basewidth,basewidth,3)
  c1 = np.mean(X_train[:,:,:,0])
  c2 = np.mean(X_train[:,:,:,1])
  c3 = np.mean(X_train[:,:,:,2])

  X1_train[:, :, :, 0] = X_train[:, :, :, 0] -c1
  X1_train[:, :, :, 1] = X_train[:, :, :, 1] -c2
  X1_train[:, :, :, 2] = X_train[:, :, :, 2] -c3
...
##=================Abre as imagens ===============
  ...
  X1_test[:, :, :, 0] = X_test[:, :, :, 0] -c1
  X1_test[:, :, :, 1] = X_test[:, :, :, 1] -c2
  X1_test[:, :, :, 2] = X_test[:, :, :, 2] -c3
...
def  load_dataset_theano_without_pretrain(num_channels, training_set_path, testing_set_path, nclass, img_rows):
  basewidth=img_rows
  src="/home/osaku/Eggs/pre/orig/"
  #==================Gera training Set
  n_train = n_samples(training_set_path)
